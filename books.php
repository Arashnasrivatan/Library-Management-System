...
<?php include('helpers/header.php'); ?>
    ...
    <?php
    if(isset($_GET['res'])){
      echo'<div style="margin:auto;" data-aos="fade-up" class="alert bgc mt-5 text-center w-50">'.htmlspecialchars($_GET['res']).'</div>';
    }
    ?>
...
$sql = "select * from books where `reserved`='1' and confirmed ='1';";
$stmt = $db->prepare($sql);
$stmt->execute();
...
if($result->num_rows == 0){
  ...
  <li>
    ...
    </div><span class="text-white">There are no books to reserve</span></div>';
}else{
...
<div class="book" data-aos="fade-up">
  ...
  <input type="text" value="'.$_COOKIE['namefull'].'" name="nameinp" class="form-control text-center" aria-label="Recipient" aria-describedby="basic-addon2" disabled>
